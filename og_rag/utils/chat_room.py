"""
utils/chatroom.py — 1-on-1 Mini Chat Room for Agrithm Bot
──────────────────────────────────────────────────────────
Features:
...
def _save_room(uid_a: int, uid_b: int, data: dict):
    with open(room_path(uid_a, uid_b), "w") as f:
...
def mark_read(uid_a: int, uid_b: int, reader_id: int):
...
def queue_offline_notification(to_id: int, from_name: str, preview: str):
    """
    ...
    msgs.append({
        "from_name": from_name,
        "preview":   preview[:80],       # short preview only

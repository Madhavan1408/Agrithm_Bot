...
Changes vs original:
  - MANDI_API_KEY loaded from environment (never hardcoded).
  - fetch_mandi_prices() tries farmer's district first, then falls back
    to state-wide search so results are never silently empty.
  - format_mandi_text() kept identical (callers already use it).
  - New async helper  handle_mandi_query()  wires voice → STT → crop
    extraction → price fetch → translate → TTS reply in one place,
    so bot.py handle_mandi_crop() becomes a thin wrapper.
"""
...
from agrithm_config import MANDI_API_URL, MANDI_API_KEY          # key from .env
...
async def handle_mandi_query(
    ...
    lang_cfg:  dict,
    *,
    # injected callables so this module stays import-free of bot internals
    download_voice_fn,
    safe_transcribe_fn,
    ...
    # ── 1. Get raw crop text from voice or text ───────────────────
    raw = ""
    ...
    # ── 3. Fetch prices ───────────────────────────────────────────
    await update.message.reply_text(
    f"Fetching mandi prices for {crop.title()} in your area..."
)
    prices = fetch_mandi_prices(crop, district)
    msg_en = format_mandi_text(prices, crop, district)

    # ── 4. Send text reply (always in English Markdown) ──────────
    await update.message.reply_text(
        msg_en,
    ...
    # ── 5. TTS reply in farmer's language ─────────────────────────
    try:
        plain = msg_en.replace("*", "").replace("_", "")
        local_text = (

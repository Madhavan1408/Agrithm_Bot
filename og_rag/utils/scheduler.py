...
FIXED:
  - Removed wrong 'from rag.utils.*' imports (was causing ImportError)
  - get_all_profiles() implemented here directly (was missing from storage.py)
  - speak() replaced with sarvam_tts() inline (speak() never existed)
  - All imports now match actual project structure
"""
...
# ═══════════════════════════════════════════════════════════════════
# get_all_profiles — was missing from storage.py, defined here
# ═══════════════════════════════════════════════════════════════════

def get_all_profiles() -> dict:
    """
    Returns dict of {user_id_str: profile_dict} for all onboarded farmers.
    Reads from data/profiles/<user_id>.json files.
    """
    profiles = {}
...
# ═══════════════════════════════════════════════════════════════════
# Sarvam TTS — inline, no dependency on bot.py
# ═══════════════════════════════════════════════════════════════════
...
def _sarvam_tts(text: str, lang_name: str) -> str | None:
    ...
    try:
        ...
        path = os.path.join(_ROOT, "audio_temp", f"digest_{datetime.now().timestamp()}.wav")
...
# ═══════════════════════════════════════════════════════════════════
# Digest Generator — simple fallback if rag module unavailable
# ═══════════════════════════════════════════════════════════════════

def _generate_digest(district: str, crop: str) -> str:
    """
    Try to use RAG digest generator; fall back to a generic message.
    This prevents scheduler from crashing when RAG module has issues.
    """
    try:
    ...
    return (
        ...
        f"• Contact your local KVK for latest scheme information.\n\n"
